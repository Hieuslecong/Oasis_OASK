...
def _excluded_paths(path):
    ...
    return {str(Path(item["path"]).resolve()) for item in data.get("excluded_normal_candidates",[])}
...
def main():
    ...
    p.add_argument("--train-ratio",type=float,default=.70)
    p.add_argument("--val-ratio",type=float,default=.15)
    p.add_argument("--seed",type=int,default=1337)
    a=p.parse_args()

    audit=json.loads(Path(a.audit_summary).read_text())
    if audit.get("status") != "PASS":
        raise RuntimeError("normal source audit must PASS before v2.1 splitting")
    excluded=_excluded_paths(a.exclude_file)
    test_ratio=1-a.train_ratio-a.val_ratio
    if not (0<a.train_ratio<1 and 0<a.val_ratio<1 and 0<test_ratio<1):
    ...
    for image in sorted(p for p in root.rglob("*") if p.is_file() and p.suffix.lower() in EXT):
        ...
        if not key or key.strip().lower() in {"unknown","none","n/a","na","placeholder"}:
    ...
    if set(train)&set(val) or set(train)&set(test) or set(val)&set(test): raise RuntimeError("normal lineage split overlap")
    ...
    out.with_suffix(out.suffix+".normal_v21.json").write_text(json.dumps(summary,indent=2))
