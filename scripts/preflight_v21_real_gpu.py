#!/usr/bin/env python3
"""Fail-closed v2.1 CUDA/data/backward preflight on real training rows."""
...
def main():
    ...
    cert=verify_gate0_certificate(
        a.gate0_certificate,a.manifest,int(cfg["image_size"]),
        "train" if a.normal_fraction>0 else "none",a.full_gate0_certificate,
    )
    ...
    critic.zero_grad(set_to_none=True); critic_total.backward()
    if not torch.isfinite(critic_total) or not finite_grad(critic):
        raise SystemExit("v2.1 critic backward is non-finite/empty")
    critic.zero_grad(set_to_none=True); critic.eval()
    for q in critic.parameters(): q.requires_grad_(False)

    arm_results={}
    for mode in ("control","cldice","aosk","connected","aosk_connected","adversarial"):
        student=make_student(a.student_kind,a.student_width).to(device)
        logits=student(x); seg=segmentation_loss(logits,y); total=seg
        if mode=="cldice": total=total+0.1*centerline_cldice_loss(logits,y)
        if mode in {"aosk","aosk_connected"}: total=total+0.01*oriented_consistency_loss(logits,x,y)
        if mode in {"connected","aosk_connected"}:
            pred=logits.sigmoid()
            with torch.no_grad(): gt=critic(x,y); corrupted=critic(x,wrong)
            rc,_=oasis_rc_student_loss_v2(critic(x,pred),gt,corrupted,pred,y)
            total=total+0.001*rc
        if mode=="adversarial": total=total+0.001*adversarial_pair_student_loss(critic(x,logits.sigmoid()))
        student.zero_grad(set_to_none=True); critic.zero_grad(set_to_none=True); total.backward()
    ...
    if git_status: raise SystemExit("preflight requires a clean git worktree")

    report={
        "status":"PASS","method":"OASIS-RC-v2.1-dev1",
        ...
        "git_head":git_head,"git_dirty":False,"dataset_content_sha256":cert["dataset_content_sha256"],
        "student_kind":a.student_kind,"student_width":a.student_width,"normal_fraction":a.normal_fraction,
        "critic_backward_loss":float(critic_total.detach()),"arms":arm_results,
