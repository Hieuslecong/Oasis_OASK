...
# Fail before allocating the model if the host cannot execute the configured CUDA run.
"$PYTHON" - "$CONFIG" "$DETERMINISM_MODE" <<'PY'
import json, os, sys, yaml, torch
cfg=yaml.safe_load(open(sys.argv[1])); mode=sys.argv[2]
