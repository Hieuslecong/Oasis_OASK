...
for f in "$CONFIG" "$MANIFEST" "$TRAIN_CERT" "$FULL_CERT"; do test -f "$f" || { echo "MISSING: $f" >&2; exit 2; }; done

mkdir -p "$EXP_ROOT/preflight" "$EXP_ROOT/init" "$EXP_ROOT/arms/B0_S0" "$EXP_ROOT/critic" "$EXP_ROOT/diagnostics"

printf '== v2.1 1/6 real GPU preflight ==\n'
"$PYTHON" "$PACKAGE_ROOT/scripts/preflight_v21_real_gpu.py" \
  ...
  --critic-width "${CRITIC_WIDTH:-8}" --out "$EXP_ROOT/preflight/${VARIANT}_seed${SEED}.json"

INIT="$EXP_ROOT/init/student_init_seed${SEED}_${STUDENT_KIND}.pt"
printf '== v2.1 2/6 shared initialization ==\n'
if [ ! -f "$INIT" ]; then
...
S0="$EXP_ROOT/arms/B0_S0/student_only.pt"
printf '== v2.1 3/6 train S0 development baseline ==\n'
if [ ! -f "$S0" ] || [ "${REUSE_S0:-0}" != "1" ]; then
  rm -f "$S0"
  "$PYTHON" -m oasis_cycle_aosk.train_oasis_rc_v21 \
    --config "$CONFIG" --manifest "$MANIFEST" --gate0-certificate "$TRAIN_CERT" --full-gate0-certificate "$FULL_CERT" \
    --out "$EXP_ROOT/arms/B0_S0" --mode control --student-kind "$STUDENT_KIND" --student-width "$STUDENT_WIDTH" \
    --epochs "${EPOCHS:-12}" --normal-fraction "$NORMAL_FRACTION" --determinism-mode "$DETERMINISM_MODE" \
    --student-init-checkpoint "$INIT" 2>&1 | tee "$EXP_ROOT/arms/B0_S0/train.log"
fi

test -f "$S0"
CRITIC="$EXP_ROOT/critic/critic.pt"
printf '== v2.1 4/6 train + qualify critic ==\n'
if [ ! -f "$CRITIC" ] || [ "${REUSE_CRITIC:-0}" != "1" ]; then
...
test -f "$CRITIC"; test -f "$EXP_ROOT/critic/critic_qualification_v21.json"
printf '== v2.1 5/6 trained-S0 RC gradient gate ==\n'
...
printf '== v2.1 6/6 B0/B1/B2/S1/S2/S3 development arms ==\n'
...
printf 'REAL_DATA_V21_DEVELOPMENT_TRAINING_READY=PASS\nVARIANT=%s\nSEED=%s\nSTUDENT_KIND=%s\nTEST_FIREWALL=CLOSED\n' "$VARIANT" "$SEED" "$STUDENT_KIND"
