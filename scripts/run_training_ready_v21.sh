...
printf '== v2.1-dev3 1/6 real GPU preflight ==\n'
...
printf '== v2.1-dev3 2/6 shared initialization ==\n'
...
printf '== v2.1-dev3 3/6 train canonical B0/S0 development baseline ==\n'
if [ -f "$S0" ] && [ "${REUSE_VALIDATED_B0:-0}" = "1" ]; then
  ...
  "$PYTHON" -m oasis_cycle_aosk.train_oasis_rc_v21 \
    ...
    --epochs "${EPOCHS:-100}" --warmup "${WARMUP:-4}" --ramp-epochs "${RAMP:-3}" \
...
printf '== v2.1-dev3 4/6 train + qualify critic ==\n'
...
printf '== v2.1-dev3 5/6 trained-B0 RC gradient/energy gate ==\n'
"$PYTHON" "$PACKAGE_ROOT/scripts/diagnose_v21_s0.py" \
  ...
  --lambda-oasis "${LAMBDA_OASIS:-0.001}" --min-s0-epochs "${MIN_S0_EPOCHS:-${EPOCHS:-100}}" \
  --out "$EXP_ROOT/diagnostics/s0_rc_${VARIANT}_seed${SEED}.json" --require-pass

printf '== v2.1-dev3 6/6 B0/B1/B2/S1/S2/S3 development arms ==\n'
...
EPOCHS="${EPOCHS:-100}" WARMUP="${WARMUP:-4}" RAMP="${RAMP:-3}" DETERMINISM_MODE="$DETERMINISM_MODE" \
...
printf 'REAL_DATA_V21_DEV3_DEVELOPMENT_PIPELINE=PASS\nVARIANT=%s\nSEED=%s\nSTUDENT_KIND=%s\nCANONICAL_B0=%s\nTEST_FIREWALL=CLOSED\n' \
