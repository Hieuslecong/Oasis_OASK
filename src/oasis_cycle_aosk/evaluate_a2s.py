"""Calibrated checkpoint evaluation for OASIS-A2S v0.1.2.
    ...
    *,
    ...
    if not match and not allow_manifest_mismatch:
        raise ValueError(
            ...
            "use --allow-manifest-mismatch only for explicit external/final evaluation"
        )
...
def _verify_dataset_content(
    ck: dict,
    manifest: str,
    split: str,
    *,
    is_dev_split: bool,
    allow_missing_lineage: bool,
    ...
    if is_dev_split:
        ...
        audit = audit_manifest(
            manifest,
            splits,
            require_lineage=not allow_missing_lineage,
            allow_size_mismatch=allow_size_mismatch,
        )
        match = audit["dataset_content_sha256"] == expected
        if not match:
            raise ValueError("development dataset bytes do not match checkpoint provenance")
        return audit, True
    audit = audit_manifest(
        manifest,
        (split,),
...
def main():
    ...
    p.add_argument("--allow-manifest-mismatch", action="store_true")
    p.add_argument("--allow-missing-lineage", action="store_true")
    p.add_argument("--allow-size-mismatch", action="store_true")
    p.add_argument("--per-image-out", default=None)
    ...
    audit, content_match = _verify_dataset_content(
        ck,
        a.manifest,
        a.split,
        is_dev_split=is_dev_split,
        allow_missing_lineage=a.allow_missing_lineage,
    ...
    arm = ck.get("arm")
    stage1 = "stage1_discriminator" in ck
    if stage1:
        model = OASISA2SDiscriminator(width, 3).to(device)
        model.load_state_dict(ck["stage1_discriminator"])
    elif "segmenter" in ck:
        model = OASISA2SDiscriminator(width, 2).to(device)
        model.load_state_dict(ck["segmenter"])
    else:
        raise ValueError("checkpoint contains neither deployable segmenter nor A1 discriminator")

    loader = DataLoader(
        ManifestDataset(a.manifest, a.split, size, return_metadata=True),
        batch_size=a.batch,
        shuffle=False,
    ...
    result = {
        **metrics,
        ...
        "arm": arm,
        "split": a.split,
        "threshold_source": ck.get("threshold_source"),
        ...
        "checkpoint_dataset_content_sha256": ck.get("dataset_content_sha256"),
        "evaluation_dataset_content_sha256": audit["dataset_content_sha256"],
        "dataset_content_provenance_match": content_match,
