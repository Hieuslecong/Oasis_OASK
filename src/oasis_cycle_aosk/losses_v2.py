...
def conditional_oasis_loss(critic_logits, target):
    ...
    return F.cross_entropy(
        critic_logits,
        target[:, 0].long(),
        weight=critic_logits.new_tensor([1.0, 8.0, 1.0]),
    )
...
def oasis_rc_critic_loss(
    ...
    crack_logit = out.get(
        "crack", out["semantic"][:, 1:2] - out["semantic"][:, 0:1]
    )
    ...
    crack_aux = (
        ...
        * valid
    ).sum() / (valid_count * crack_target.shape[-2] * crack_target.shape[-1])
    ...
    mismatch = F.binary_cross_entropy_with_logits(
        out["mismatch"], mismatch_target, pos_weight=pw
    )
    ...
    clean_mismatch = (
        F.binary_cross_entropy_with_logits(
            out["mismatch"],
            torch.zeros_like(mismatch_target),
            reduction="none",
        )
        * valid_crack_pixels
    ).sum() / valid_crack_count

    pair = F.binary_cross_entropy_with_logits(
        out["pair"], pair_valid, pos_weight=out["pair"].new_tensor(1.0)
    )
...
def oasis_rc_student_loss(pred_out, gt_out, student_mask, target, margin=0.10):
    ...
    e_pred = q_pred.mean((1, 2, 3)) + 0.25 * (
        -pred_out["pair"].sigmoid().squeeze(1)
    )
...
def relation_energy(out, pair_weight=0.25):
    ...
    This is public so diagnostics use the exact same energy definition as the
    OASIS-RC-v2 student objective rather than reimplementing it separately.
    """
...
def _relation_energy(out, pair_weight=0.25):
    ...
    return relation_energy(out, pair_weight=pair_weight)
...
def oasis_discriminator_loss(logits_real, semantic, logits_fake):
    ...
    fake_target = torch.full(
        logits_fake.shape[:1] + logits_fake.shape[2:],
        2,
