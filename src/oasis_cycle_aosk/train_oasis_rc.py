"""OASIS-RC controlled experiment.
...
from .losses import segmentation_loss, oasis_rc_critic_loss, oasis_rc_student_loss
...
def train_critic(args, cfg, device, out):
    ...
    for epoch in range(args.critic_epochs):
        ...
        for x, y in loader:
            ...
            opt.zero_grad()
            terms = [oasis_rc_critic_loss(critic(x, y), sem, mm, pv)
                    + oasis_rc_critic_loss(critic(x, wrong), sem_w, mm_w, pv_w)
                    + oasis_rc_critic_loss(critic(rgb_bad, y), sem_rgb, mm_rgb, pv_rgb)
                    + oasis_rc_critic_loss(critic(x, mask_bad), sem_m, mm_m, pv_m)]
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in train_loader:
            ...
            if critic is not None and epoch >= args.warmup:
                ...
                with torch.no_grad(): gt_out = critic(x, y)
                pred_out = critic(x, pred_mask)
                rc, extras = oasis_rc_student_loss(pred_out, gt_out, pred_mask, y)
