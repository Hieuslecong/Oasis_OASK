...
def make_student(kind, width):
    if kind not in VALID_STUDENT_KINDS:
    ...
    raise AssertionError("validated student kind was not handled")
...
def critic_metrics(critic, loader, device, normal_loader=None):
    ...
    per_kind = {name: [0.0, 0.0, 0] for name in CORRUPTION_NAMES}
    donor_bank = []
    ...
    if normal_loader is not None:
        ...
        for batch in normal_loader:
            ...
            if donor_masks is not None and donor_masks.shape[0] > 0:
                ...
                per_kind["C8_crack_on_normal"][2] += int(xn.shape[0])
    ...
    corruption_samples = {}
    all_tp = all_fn = 0.0
    for name, (tp, fn, samples) in per_kind.items():
        corruption_recall[name] = (
            tp / (tp + fn + 1e-8) if tp + fn > 0 else None
        )
        all_tp += tp
        all_fn += fn
        corruption_samples[name] = int(samples)
...
def train_critic(args, cfg, device, out, determinism_mode):
    ...
    for epoch in range(args.critic_epochs):
        ...
        for x, y, is_normal in loader:
            ...
            if crack_rows.any():
                xc, yc = x[crack_rows], y[crack_rows]
                rgb_shuffle_samples += int(yc.shape[0])
        ...
        row = {
            ...
            "rgb_shuffle_samples": rgb_shuffle_samples,
            "mask_flip_training": False,
        }
        history.append(row)
    ...
    checkpoint = {
        ...
        "full_gate0_certificate_sha256": sha256_file(
