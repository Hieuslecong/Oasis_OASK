...
def row(tmp_path, name, split, empty=False):
    image = tmp_path/f"{name}.png"
    mask = tmp_path/f"{name}_m.png"
    Image.new("RGB", (16,16), color=(20,30,40)).save(image)
...
def test_builder_excludes_unreviewed_empty_and_freezes_actual_hashes(tmp_path):
    rows = [row(tmp_path, "tr", "train"), row(tmp_path, "va", "val"),
            row(tmp_path, "te", "test"), row(tmp_path, "empty", "train", True)]
