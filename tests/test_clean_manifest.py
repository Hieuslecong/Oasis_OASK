...
from scripts.clean_manifest import run, strip_lineage
...
def row(i, m, split, lineage):
    return {"image": str(i), "mask": str(m), "split": split,
            "source_id": "s", "lineage_id": lineage, "is_normal": False}
...
def test_lineage_priority_preserves_test(tmp_path):
    ...
    for p, v in ((ti, 10), (vi, 20), (ei, 30)): rgb(p, v)
    for p, x in ((tm, 2), (vm, 6), (em, 10)): mask(p, x)
    rows = [row(ti, tm, "train", "train:parent"),
...
def test_nonempty_mask_reuse_drops_lower_priority(tmp_path):
    ...
    for p, v in ((ti, 10), (vi, 20), (ei, 30)): rgb(p, v)
    mask(shared, 3); mask(em, 10)
    rows = [row(ti, shared, "train", "train:a"),
